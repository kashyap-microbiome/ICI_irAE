zicoseq_volcano_plot <- function(zico.obj, grp.name, p.cutoff, top.n, p_value_class, log2FC=NA, save_plot = FALSE, plot_name="") {
  ...
  output_plot <- ggplot(df, aes(x = R2.2, y = y_vals)) + 
    ...
    labs(title = paste0(plot_name, " ", grp.name)) +
    labs(caption = paste0("raw or adj. p-values = ", p_value_class, "  ;  cutoff = ", p.cutoff))
  
  
  if (save_plot == TRUE) {
    filename <- paste0("../figures/", "volcano ", plot_name, grp.name ,".png")
