zicoseq_heatmap_func <- function(zico_list, grp.names, grp.labels, sign_cutoffs, single_cutoff = FALSE, p_or_q, update_names="No",
                                 update_df="", rotate=F, column_name_rot = 0, left_margin=4) {
  ...
  #-----------------------------------------------------------------------------
  #updating names
  if (class(update_df) == "character" & !update_names == "No") {print("please provide a dataframe to update names")}
  
  if (update_names == "KO") {
    rownames(update_df) <- make.unique(update_df$KO)
    name_vec <- update_df[rownames(R2.df), "name_split_KO"]
    name_vec[is.na(name_vec)] <- rownames(R2.df)[is.na(name_vec)]
    name_vec <- gsub(" NA", "", name_vec)
    rownames(R2.df) <- name_vec
  }
  
  if (update_names == "EC") {
    rownames(update_df) <- make.unique(update_df$EC_number)
    name_vec <- update_df[rownames(R2.df), "name"]
    name_vec[is.na(name_vec)] <- rownames(R2.df)[is.na(name_vec)]
    name_vec <- gsub(" NA", "", name_vec)
    rownames(R2.df) <- name_vec
  }
  ...
  lgd <- Legend(1:3, title=legend_name, labels = sign_cutoffs, type = "points", pch=rev(c('***', '**', '*')), background = "white")
  ...
  heatmap_plot <- Heatmap(R2.df, name = "R2", 
          ...
          column_names_max_height = max_text_width(colnames(R2.df), gp = gpar(fontsize = 18))) 
  ...
  return(list(heatmap_plot = heatmap_plot, sign_names = rownames(R2.df), lgd=lgd))
